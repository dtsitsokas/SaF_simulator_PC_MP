function gplotdc2(A,xy,varargin)
...
% 4 cases of colors 
if (X1(ind,1)==0) %network not in subnetwork
    cc=[0.75 0.75 0.75];
elseif (X1(ind,1)==1) %subnetwork 
    cc=[0 0 1];
elseif (X1(ind,1)==2)
    cc=[1 0 0];
else
...
hold on
% COLOR=['b','r','c','k','m','g'];
% plot(dX,dY,'color',(1-(X1(ind,1)/(100)))*[1 1 1],params,'Linewidth',1.5)
dX1=dX(~isnan(dX));
dY1=dY(~isnan(dY));
plot(dX1,dY1,'color',cc,params,'Linewidth',1.5)
% plot(ix,iy,'ko',params)
plot(xy(:,1),xy(:,2),'k.','Markersize',0.5)
hold off
