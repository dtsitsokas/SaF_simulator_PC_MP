function [] = printNodeClustering(nodesC, mode_n, PC, no_adjReg)
% nodesC = Set of nodes (ID) that I want to highlight on the map 
% mode_n = 0 for all nodes of the cluster (needs the PC struct), 1 for
% specific node 
% PC = the PC struct with all PC info 
...
%control = readmatrix("PCjunctionsID.csv"); 
...
PCnodeSize = 12;
if mode_n == 0
    %print all nodes of the PC.nodes set 
    ...
    %print one specific node 
    plot(nodes(nodes(:,1)==nodesC,2),nodes(nodes(:,1)==nodesC,3),...
                    'Marker','o','MarkerFaceColor','#7E2F8E','MarkerEdgeColor','#A2142F','Markersize',PCnodeSize)
