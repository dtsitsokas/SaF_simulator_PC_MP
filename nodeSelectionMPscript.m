%function [MPnodes] = nodeSelectionMP(demandCode,a,b,case_j,equation,caseName)
...
case_j = 1;
equation = 1;
caseName = 'test1'; %not saving MPnode file (uncomment lines 


%% reference case file name
if demandCode == 1
    demand = 'med';
    fname = 'C:\Users\tsitsoka\Documents\Dimitris Local Documents pc10\GitHub-My repositories\MaxPressure_Codes-\Current results\ResultsMediumOD_capacityDrop\output_set5_MedDemand_NC';
elseif demandCode == 2
...
title('peak period (2h)')
xlabel('avg node variance')
ylabel('duration of node congestion (cycles)')
hold on
...
for reg=1:3
    ...
    if mode_A == 1
        ...
        if ~mode_C==1
            if equation==2
                %eq 2:
                A{reg} =  1.2*a*(0.15-node_queues{reg}) - b*1.2*(node_var{reg}-0.03)/0.6 - abs(1-a-b)/80*node_congested{reg}; %80 = number of cycles in the defined peak hour
            elseif equation==1
                %eq 1:
                A{reg} = -1.2*a*(0.9-node_queues{reg}) -1.2*b*(node_var{reg}-0.03)/0.6 - abs(1-a-b)/80*node_congested{reg}; %80 = number of cycles in the defined peak hour
            end
        else
...
if mode_A == 1
    ...
    if mode_C==1
        ...
        figure(f2)
        scatter(node_varAll(Ib(1:num_Top)), node_congAll(Ib(1:num_Top)),'filled')
        scatter(node_varAll(Ib(num_Top+1:end)), node_congAll(Ib(num_Top+1:end)),'filled','MarkerFaceColor','y')
        %saveas(gcf,strcat('sel_',num2str(1*(mode_A==0)+2*(mode_A==1)+3*(mode_C==1)),'_',num2str(round(100*target_p)),'_',demand,'_','b.fig'));
    end
end
