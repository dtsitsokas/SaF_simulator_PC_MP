...
MPnodeSelection = 2; %1 for the proposed methodology, 2 for the benchmark (ignore if no MP) 
...
inputPCfile_med = strcat(cd,'/inputPCscenarios/input_PC_62'); %'C:\Users\tsitsoka\Documents\Dimitris Local Documents pc10\GitHub-My repositories\MaxPressure_Codes-\input files\runs high demand\final runs input files\input_PC_62.mat';
inputPCfile_high = strcat(cd,'/inputPCscenarios/input_PC_50b'); %'C:\Users\tsitsoka\Documents\Dimitris Local Documents pc10\GitHub-My repositories\MaxPressure_Codes-\input files\runs medium demand\input_PC_50b.mat'; 


%*** load new indata (debugging) 
path_in_med = cd; %'C:\Users\tsitsoka\Documents\Dimitris Local Documents pc10\GitHub-My repositories\MaxPressure_Codes-\';
path_in_high = cd; %'C:\Users\tsitsoka\Documents\Dimitris Local Documents pc10\GitHub-My repositories\MaxPressure_Codes-\';
% path_out_high = 'C:\Users\tsitsoka\Documents\Dimitris Local Documents pc10\GitHub-My repositories\MaxPressure_Codes-\';
% path_out_med = 'C:\Users\tsitsoka\Documents\Dimitris Local Documents pc10\GitHub-My repositories\MaxPressure_Codes-\';

%set the file name of the results of the reference case for MP node
%selection (use NC case)
reffname_med = strcat(cd,"/results med 2025/output_final_med_NC.mat"); %'C:\Users\tsitsoka\Documents\Dimitris Local Documents pc10\GitHub-My repositories\MaxPressure_Codes-\output_final_med_NC.mat';
reffname_high = strcat(cd,"/results high 2025/output_final_high_NC.mat"); %'C:\Users\tsitsoka\Documents\Dimitris Local Documents pc10\GitHub-My repositories\MaxPressure_Codes-\output_final_high_NC.mat';

% paths where to save results: 
path_out_med = strcat(cd,'/results med 2025/'); %'F:\Max Pressure files\fifth set results\ResultsMediumOD_capacityDrop\';
path_out_high = strcat(cd,'/results high 2025/'); %'F:\Max Pressure files\fifth set results\ResultsHighOD_capacityDrop\';
...
for k = 1:length(jj)
    ...
    if dataChanged == 1
        prepareInput(demandCode);  %run when there changes in the network/demand etc. 
    end
    ...
    if MPcode==1
        ...
        elseif MPnodeSelection == 2
            ...
            [~] = nodeSelectionMP_outflow(demandCode, a,b,case_j,equation,scenarios{j}); 
        end
    end
    ...
    load(strcat(cd,'/input_',scenarios{j},'.mat'))
