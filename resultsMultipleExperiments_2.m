...
% Script for the case of REDUCED result files (only VHT saved) - HIGH OD
...
% path of the result files of the reduced random experiments 
path = 'F:\Max Pressure files\fifth set results\ResultsHighOD_capacityDrop\RandomMPonly_Results\';
...
%case same a,b as in medium (0.5, 1.5)
% outfnameProtheme = 'output_set5_highDemand_MP_cd_high_MS'; %name of result of selection cases 
% inpfnameProtheme = 'input_MP_cd_high_MS';

outfnameProtheme = 'output_set5_highDemand_MP_grid_high_eq2t_MS'; %name of result of selection cases 
inpfnameProtheme = 'input_MP_grid_high_eq2t_MS';
outfnameProtheme2 = 'output_final_high_MP_cd_MSR';
...
for j =1:length(perc)
    ...
    for i=1:10 %replications 
        ...
        load(strcat(path,'output_MP_R_',num2str(perc(j)),'_',num2str(i),'.mat'))
...
for j=0:length(perc)-1
   ...
   load(strcat(pathNC,outfnameProtheme,num2str(j)),'outdata')
   %load(strcat(pathIn,inpfnameProtheme,num2str(j)),'p_1')
   load(strcat(inpfnameProtheme,num2str(j)),'p_1')
   %store VHT      
        % Table of Results
        i = j + 1; 
        %Total time spent in network and VQs -
...
%load results file
load(strcat(pathNC,'output_set5_highDemand_MP_0'),'outdata')
p_1 = 100;
%store VHT
% Table of Results
i = length(perc) + 1;
...
%% Load scenarios of selected nodes from benchmark (max outflow) 
Sresults = zeros(10,length(perc));
for j=0:length(perc)-1
   %load results file - set file name convention 
   load(strcat(pathNC,outfnameProtheme2,num2str(j)),'outdata')
   p_1 = 0.05 + j*0.05; 
   %store VHT      
        % Table of Results
        i = j + 1; 
...
saveas(gcf,strcat(pathFig,'Fig_1b_MP_HighDemand_a.fig'));
saveas(gcf,strcat(pathFig,'Fig_1b_MP_HighDemand_a.emf'));
print(gcf,strcat(pathFig,'Fig_1b_MP_HighDemand_a.eps'),'-depsc','-painters')

%figure(f1)
%scatter((1:length(perc)), med_TTT(1:length(perc)),'*','SizeData',60)
%scatter((1:length(perc)), med_TTT(length(perc)+1:end-1),'filled','SizeData',60)
%scatter(length(perc)+1, med_TTT(end),'filled','SizeData',60)
%ylim auto
%xlim([0,length(perc)+2])
...
%title('high demand')
%legend('random MP node sets (median)','MP to selected nodes','MP to all nodes','Fixed-Time Control','Location','northeast')
