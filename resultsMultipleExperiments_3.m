...
% Script for the case of REDUCED result files (MEDIUM OD) 
...
%pathNC = 'C:\Users\tsitsoka\Documents\Dimitris Local Documents pc10\GitHub-My repositories\MaxPressure_Codes-\Current results\ResultsMediumOD_capacityDrop\';
...
path = 'F:\Max Pressure files\fifth set results\ResultsMediumOD_capacityDrop\RandomSetsResults\MP plus PC - reduced results\';
...
perc = [5 10 15 20 25]; 
demand = 'medium'; 
figfName = 'Fig_2aPC_PCMP_MedDemand';
fname_PCMP100 = 'output_final_med_PC_620'; 
fname_PC = 'output_final_med_PC_62'; 
fname_prothema_PCMPR = 'output_PC_62_MP_R'; %format fname of random PC_MP results 
%fnameinput_prothema_MPMS = 'input_PC_50b_MP_MS';
fnameinput_prothema_MPMS = strcat(pathIn,'input_PC_62_MPnc_MS');
% fnameOutput_prothema_PCMPMS = 'output_set5_HighDemand_PC_50b_MP_MS';
fnameOutput_prothema_PCMPMS = 'output_final_med_PC_62_MP_MS';
fname_prothema_benchmark = 'output_final_med_PC_62_MP_MSR' ;
...
for j=0:length(perc)-1
   ...
   %load(strcat(fnameinput_prothema_MPMS,num2str(j)),'p_1')
   p_1 = 0.05 + 0.05*j; 
   %store VHT      
        % Table of Results
        i = j + 2; 
...
%% Load scenarios of selected nodes by benchmark (max outflow) 
Sresults = zeros(10,length(perc));
for j=0:length(perc)-1
   %load results file - set file name convention 
   load(strcat(pathNC,fname_prothema_benchmark,num2str(j)),'outdata')
   %load(strcat(fnameinput_prothema_MPMS,num2str(j)),'p_1')
   p_1 = 0.05 + 0.05*j; 
   %store VHT      
        % Table of Results
...
scatter((2:length(perc)+1), med_imp(length(perc)+2:2*length(perc)+1),'v','filled','SizeData',60) %result of PC + MP method  
...
legend('single PC',' PC + MP to random node sets (median)','PC + MP (proposed)','PC + MP to all nodes','Fixed-Time Control','Location','northeast')
...
print(gcf,strcat(pathFig,figfName,'.eps'),'-depsc','-painters')
