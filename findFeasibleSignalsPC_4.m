function [out_1,out_2] = findFeasibleSignalsPC_4(agg_u, greens_P, avg_x ,or_index, gn_R, PC, ind, twin_ind, minG, sat_flows,alpha,beta)
...
%     satFlow: Saturation flow of all links 
% Returns 
...
satFlow = zeros(size(greensPC_prev));
...
%     queues(i) = sum(queue_set);
    ...
    queues(i) = sum(avg_x(or_index(approaches))); %the queue of node i is the average gueue of all approaches of primary phase of i 
    
    %Saturation flows of every node = sum of Sat flows of all approaches of
    %the primary phase
    satFlow(i) = sum(sat_flows(or_index(approaches)));
...
%     queues(i+length(ind)) = sum(queue_set); 
    if sum(approaches)>0 
        ...
        satFlow(i+length(ind)) = sum(sat_flows(or_index(approaches)));
    else
        %for dynamic PC in case secondary is an All-Red phase - same as
        %primary phase 
        queues(i+length(ind)) = queues(i);
        satFlow(i+length(ind)) = satFlow(i);
    end
end
...
%(cycle maintained) 
Const = [x-minGreens >= 0, (-maxFluct <= x-greensPC_prev),x-greensPC_prev <= maxFluct,...
    x(1:length(ind))+x(length(ind)+1:end)-sumgreens==0]; 
% -theta <= mean(x(1:length(ind)))-agg_u <= theta
...
%best working so far for static and dynamic PC: 
Obj = alpha*(x-greensPC)*(Q+1)*(x-greensPC)' + beta*(ones(size(x))-x./(diag(Q+1)'))*(Q+1)*((ones(size(x))-(x.*satFlow)./(diag(Q+1)'))');

%(tested - worse)
%Obj = alpha*(sum(x(1:length(ind)).*satFlow(1:length(ind)))-agg_u*sum(satFlow(1:length(ind))))^2; %+  ...
   %+ beta*(ones(size(x))-x./(diag(Q+1)'))*(Q+1)*((ones(size(x))-(x.*satFlow)./(diag(Q+1)'))');

%(the one in paper if static PC- slightly worse) 
%Obj = alpha*(sum(x(1:length(ind)))-agg_u*length(ind))^2 +  ...
    %+ beta*(ones(size(x))-x./(diag(Q+1)'))*(Q+1)*((ones(size(x))-(x.*satFlow)./(diag(Q+1)'))');

%Obj = (ones(size(x))-x./(diag(Q+1)'))*(Q+1)*((ones(size(x))-(x.*satFlow)./(diag(Q+1)'))');

%Q = queues;

% Set some options for YALMIP and solver
opt = sdpsettings('verbose',1,'solver','gurobi','gurobi.TimeLimit',3,'gurobi.TuneTimeLimit',3,'verbose',0);

% Solve the problem
sol = optimize(Const,Obj,opt);

% Analyze error flags
if sol.problem == 0 ||  sol.problem == 9 || sol.problem == 3
...
%(newGreens-greensPC)*Q*(newGreens-greensPC)' + (ones(size(newGreens))-greensPC./(diag(Q+1)'))*((ones(size(newGreens))-greensPC./(diag(Q+1)'))');
